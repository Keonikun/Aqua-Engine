...
export class StaticWorldCollider {
  constructor(config = {}) {
    ...
    this.traceIterations = Math.max(1, Math.floor(readNumber(config.traceIterations, 6)))
    this.maxTraceStepLength = Math.max(readNumber(config.maxTraceStepLength, 0.04), EPSILON)
    this.contactSlop = Math.max(readNumber(config.contactSlop, 0.02), 0)
    this.stuckTestEpsilon = Math.max(readNumber(config.stuckTestEpsilon, 0.002), 0)
  }
  ...
  buildFromSceneObject(sceneObject) {
    ...
    sceneObject.traverse((child) => {
      ...
      if (child.userData.collisionKind === 'brush' && isAxisAlignedMesh(child)) {
